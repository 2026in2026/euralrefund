    ...
    <div>
      ...
      <button
        ...
        style={{
          ...
          color: file && !loading ? "#FAFAF8" : "#9090A0",
    ...
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 2fr", gap: 12, marginTop: 24 }}>
        ...
        <button onClick={onNext} disabled={!canProceed} style={{
          ...
          color: canProceed ? "#FAFAF8" : "#9090A0", border: "none", borderRadius: 8,
      ...
      {eligible && (
        <>
          ...
          <div style={{ background: "#FFFFFF", border: "1px solid #3a3a5e", borderRadius: 10, padding: 16, marginBottom: 24 }}>
            <div style={{ color: "#4A4A5A", fontSize: 11, fontFamily: "'DM Mono', monospace", marginBottom: 8, letterSpacing: "0.1em", textTransform: "uppercase" }}>Klageinstans</div>
            <div style={{ color: "#1C1C28", fontFamily: "'DM Mono', monospace", fontSize: 14 }}>{op?.authority}</div>
...
function FormStep({ extractedInfo, compensation, onBack }) {
  ...
  if (subStep === "details") return (
    <div>
      ...
      <div style={{ display:"grid", gap:14 }}>
        <div>
          ...
          {ticketName && !nameMismatch && name.trim().length > 2 && (
            <div style={{ color:"#2D8653", fontSize:11, marginTop:4, fontFamily:mono }}>
              ✓ Name matches ticket
            </div>
          )}
        </div>
        ...
        <div
          ...
          style={{ display:"flex", gap:10, cursor:"pointer", alignItems:"flex-start", padding:"12px", background: gdprConsent ? "#F0FAF4" : "#FFFFFF", border: `1px solid ${gdprConsent ? "#A8D8B8" : "#E0DCD4"}`, borderRadius:8, transition:"all 0.2s" }}
        >
        ...
        <div
          ...
          style={{ display:"flex", gap:10, cursor:"pointer", alignItems:"flex-start", padding:"12px", background: agreed ? "#F0FAF4" : "#FFFFFF", border: `1px solid ${agreed ? "#A8D8B8" : "#E0DCD4"}`, borderRadius:8, transition:"all 0.2s" }}
        >
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 2fr", gap:12, marginTop:20 }}>
        <button onClick={onBack} style={{ padding:"13px", background:"transparent", border:"1px solid #3a3a5e", borderRadius:8, color:"#4A4A5A", cursor:"pointer", fontFamily:mono, fontSize:13 }}>
          ← Tilbage
        </button>
        <button
          onClick={() => setSubStep("sign")}
          disabled={!canGo}
          style={{ padding:"13px", background: canGo ? "#C8A96E" : "#F0EDE8", color: canGo ? "#FAFAF8" : "#9090A0", border:"none", borderRadius:8, cursor: canGo ? "pointer" : "not-allowed", fontFamily:mono, fontSize:13, fontWeight:700, transition:"all 0.2s" }}
        >
  ...
  if (subStep === "sign") return (
    <div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 2fr", gap:12 }}>
        <button onClick={() => setSubStep("details")} style={{ padding:"13px", background:"transparent", border:"1px solid #3a3a5e", borderRadius:8, color:"#4A4A5A", cursor:"pointer", fontFamily:mono, fontSize:13 }}>
          ← Tilbage
        </button>
        <button
          onClick={doGenerate}
          disabled={!signed || loading}
          style={{ padding:"13px", background: signed && !loading ? "#C8A96E" : "#F0EDE8", color: signed && !loading ? "#FAFAF8" : "#9090A0", border:"none", borderRadius:8, cursor: signed ? "pointer" : "not-allowed", fontFamily:mono, fontSize:13, fontWeight:700 }}
        >
