  ...
  return (
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 2fr", gap: 12, marginTop: 24 }}>
        <button onClick={onBack} style={{ padding: "14px", background: "transparent", border: "1px solid #E0DCD4", borderRadius: 8, color: "#4A4A5A", cursor: "pointer", fontFamily: "'DM Mono', monospace", fontSize: 13 }}>
          ← Back
        </button>
...
function FormStep({ extractedInfo, compensation, onBack }) {
  ...
  const inp = { width:"100%", padding:"11px 13px", background:"#FFFFFF", border:"1px solid #E0DCD4", borderRadius:8, color:"#1C1C28", fontFamily:mono, fontSize:13, outline:"none", boxSizing:"border-box" };
  ...
  const doGenerate = async () => {
    ...
    try {
      ...
      // Submit claim to backend (fire-and-forget — don't block on failure)
      try {
        const submitRes = await fetch("/api/submit-claim", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            name, email, address, iban,
            ...
            ourFee: compensation.ourFee,
            youGet: compensation.youGet,
          }),
        });
        const submitData = await submitRes.json();
        if (submitData?.ref) setClaimRef(submitData.ref);
      } catch (submitErr) {
        console.warn("Claim submission email failed:", submitErr);
        // Non-fatal — PDFs already downloaded
      }
  ...
  if (subStep === "details") return (
    <div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 2fr", gap:12, marginTop:20 }}>
        <button onClick={onBack} style={{ padding:"13px", background:"transparent", border:"1px solid #E0DCD4", borderRadius:8, color:"#4A4A5A", cursor:"pointer", fontFamily:mono, fontSize:13 }}>← Back</button>
  ...
  if (subStep === "sign") return (
    <div>
      ...
      <p style={{ color:"#4A4A5A", fontSize:13, marginBottom:20, fontFamily:mono }}>Type your full name to sign both documents</p>
      <div style={{ background:"#FAFAF8", border:"1px solid #E0DCD4", borderRadius:10, padding:18, marginBottom:20 }}>
        <div style={{ fontFamily:mono, fontSize:10, color:"#4A4A5A", marginBottom:12, letterSpacing:"0.1em" }}>YOU ARE SIGNING</div>
        {[
          ...
          <div key={title} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"8px 0", borderBottom:"1px solid #E0DCD4" }}>
            <div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 2fr", gap:12 }}>
        <button onClick={() => setSubStep("details")} style={{ padding:"13px", background:"transparent", border:"1px solid #E0DCD4", borderRadius:8, color:"#4A4A5A", cursor:"pointer", fontFamily:mono, fontSize:13 }}>← Back</button>
        <button onClick={doGenerate} disabled={!signed || loading}
          ...
          {loading ? "⟳  Generating documents..." : "✍️  Sign and download PDF →"}
        </button>
      </div>
    </div>
  );
  ...
  return (
    <div style={{ textAlign:"center", padding:"10px 0" }}>
      ...
      <p style={{ fontFamily:mono, fontSize:13, color:"#4A4A5A", marginBottom:20, lineHeight:1.8 }}>
        Your documents have been downloaded and we have received your claim. We will file everything on your behalf — you don't need to do anything else.
      </p>

      {claimRef && (
        <div style={{ background:"#F0FAF4", border:"1px solid #A8D8B8", borderRadius:10, padding:"12px 16px", marginBottom:20 }}>
          <div style={{ fontFamily:mono, fontSize:10, color:"#2D8653", letterSpacing:"0.15em", marginBottom:4 }}>YOUR CLAIM REFERENCE</div>
          <div style={{ fontFamily:"'Playfair Display', serif", fontSize:22, color:"#1C1C28", letterSpacing:"0.05em" }}>{claimRef}</div>
          <div style={{ fontFamily:mono, fontSize:11, color:"#4A4A5A", marginTop:4 }}>A confirmation has been sent to {email}</div>
        </div>
      )}
      ...
      <p style={{ fontFamily:mono, fontSize:11, color:"#9090A0", marginTop:16 }}>
      ...
      <div style={{ width: "100%", maxWidth: 520, position: "relative" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 24, marginBottom: 20 }}>
            ...
            <div>
              ...
              <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 11, color: "#4A4A5A", lineHeight: 2.2 }}>
                ...
                <div>CVR: [Pending registration]</div>
        ...
        <div style={{ textAlign: "center", marginTop: 16 }}>
          ...
          <div style={{ display: "flex", justifyContent: "center", gap: 20, flexWrap: "wrap" }}>
            ...
            <span style={{ color: "#E0DCD4" }}>·</span>
            <span style={{ fontFamily: "'DM Mono', monospace", fontSize: 10, color: "#B0ADA8" }}>EU Rail Refund ApS · Denmark</span>
          </div>
        </div>
      </div>
      <Head>
